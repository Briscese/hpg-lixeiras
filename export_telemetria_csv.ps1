...
$output = Join-Path $PSScriptRoot ("telemetria-{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))

if (-not (Test-Path -LiteralPath $database)) {
    throw "Banco local nao encontrado: $database"
}

python -c "import csv, sqlite3, sys; db, output = sys.argv[1:]; connection = sqlite3.connect(db); cursor = connection.execute('SELECT * FROM telemetry_records ORDER BY received_at_ms'); writer = csv.writer(open(output, 'w', newline='', encoding='utf-8-sig')); writer.writerow([column[0] for column in cursor.description]); writer.writerows(cursor); connection.close()" $database $output
