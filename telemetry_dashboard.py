...
<title>Telemetria Teltonika</title><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" integrity="sha256-p4NxAoJBhIINfQ7l5QyQ8V2Llf4H2qR+2zC6F5F5h8Q=" crossorigin=""><style>
body{margin:0;background:#f3f6f8;color:#17283b;font:14px Arial,sans-serif}header{background:#073b75;color:#fff;padding:17px 24px;display:flex;gap:18px;align-items:center}h1{font-size:20px;margin:0}#updated{opacity:.85;font-size:12px}main{max-width:1300px;margin:auto;padding:20px 24px}.toolbar{display:flex;flex-wrap:wrap;gap:12px;align-items:end;margin-bottom:15px}label{display:grid;gap:5px;font-size:12px;font-weight:bold}input,select,button{padding:8px 10px;border:1px solid #bcc8d3;border-radius:4px;font:inherit;background:#fff}button{background:#0865b2;border-color:#0865b2;color:#fff;cursor:pointer}.summary{display:grid;grid-template-columns:repeat(3,minmax(160px,1fr));gap:12px;margin-bottom:15px}.metric,.table,.map-panel{background:#fff;border:1px solid #d8e0e8;border-radius:5px}.metric{padding:13px}.metric span{display:block;color:#5a6b7e;font-size:12px}.metric strong{font-size:19px}.map-panel{margin-bottom:15px;overflow:hidden}.map-title{padding:10px 12px;border-bottom:1px solid #d8e0e8;font-weight:bold}#map{height:410px}.table{overflow:auto}table{border-collapse:collapse;width:100%;min-width:930px}th,td{padding:10px 12px;border-bottom:1px solid #e1e7ed;text-align:left;white-space:nowrap}th{background:#edf2f6;color:#415366;font-size:12px}.yes{color:#087441;font-weight:bold}.no{color:#a43a27}.empty{padding:24px;color:#5a6b7e}a{color:#075da9}@media(max-width:640px){main{padding:15px}.summary{grid-template-columns:1fr}#map{height:320px}}
...
<section class="summary"><div class="metric"><span>Registros exibidos</span><strong id="count">-</strong></div><div class="metric"><span>Última posição válida</span><strong id="last">-</strong></div><div class="metric"><span>Banco</span><strong id="database">-</strong></div></section><section class="map-panel"><div class="map-title">Posições GPS válidas</div><div id="map"></div></section>
...
</main><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" integrity="sha256-20nQCchB9co0qIjJZRGuk2/Z9VM+kNiyxNV1lvTlZBo=" crossorigin=""></script><script>
